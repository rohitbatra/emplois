...
window.open(
  \'https://sezplus.com/jobs/resume-view/?id=\' + id,
  \'_blank\'
);
}
</script>
...
$thirty_days_ago = time(strtotime('-30 days'));
$last_login_time = $user_meta['cjfm_last_login'][0];
$inactive = false;
if($thirty_days_ago < $last_login_time)
{
	// inactive
    $inactive = true;
	echo '
...
if ($result->num_rows > 0)
{
	...
	//$chck_apr_sql = "SELECT ajb.user_ID FROM applied_job_details AS ajb LEFT OUTER JOIN wp_posts ON (wp_posts.ID = job_ID) WHERE ajb.user_ID = '{$user_ID}' AND (approved_by_candidate = '1' OR approved_by_company ='1') AND emp_id = '{$emp_id}';";
    $result = mysqli_query($conn, $chck_apr_sql);
    if($result->num_rows > 0)
    {
        echo '
        <img style="margin-bottom:12px;" src="https://sezplus.com/jobs/wp-content/uploads/2016/04/watermark.png"/>
...
if(!$inactive)
{
    ...
    {
$duplicate_check = "SELECT ajb.user_ID FROM applied_job_details AS ajb LEFT OUTER JOIN wp_posts ON (wp_posts.ID = job_ID) WHERE ajb.user_ID = '{$user_ID}' AND (approved_by_candidate = '1' OR approved_by_company ='1') AND wp_posts.post_author = '{$emp_id}';";
//$duplicate_check        =       "SELECT * FROM applied_job_details WHERE user_ID = '$user_ID' AND emp_id = '$emp_id' AND sent_by_emp = '1' ;";
$result = mysqli_query($conn, $duplicate_check);
...
echo '
        ...
        if(!isset($_GET['referrer']) && $_GET['referrer'] !== 'c_list')
	{
	echo '
