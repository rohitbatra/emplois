...
function cjfm_social_update_existing_user($social_profile, $form_id, $redirect){
	...
	update_user_meta($user_info['ID'], 'cjfm_social_avatar', $social_profile->photoURL);
...
function cjfm_social_register_user($social_profile, $form_id, $redirect){
	...
	if($user_login == ''){
		$user_login = cjfm_generate_unique_username($social_profile->identifier);
	}
	...
	if($user_email == '@no-email.com'){
		$user_email = cjfm_generate_dummy_email($social_profile->identifier);
	}
	...
	if(!is_wp_error($new_user_id)){
		...
		update_user_meta($new_user_id, strtolower($service).'_url', $social_profile->photoURL);
