...
function cjfm_admin_notices(){
	...
	if($count != array_sum($page_setup)){
		...
		echo '<div class="updated error" style="margin-top:10px;">
...
function cjfm_auto_page_setup(){
	...
	if(isset($_GET['cjfm_do_action']) && $_GET['cjfm_do_action'] == 'create_pages'){
		...
		$pages['page_login'] = array(
			...
			'post_content'   => '[cjfm_form_login form_id="1" redirect_url="" user_login_label="Username or Email address:" user_login_placeholder="" user_pass_label="Password:" user_pass_placeholder="" show_icons="yes" show_remember_me="yes" required_text="*" button_text="Login" forgot_password_text="Forgot password?" button_class="" class="" ]',
			'post_name'      => 'login',
		...
		$pages['page_register'] = array(
			...
			'post_content'   => '[cjfm_form_register form_id="1" redirect_url="" required_text="*" button_text="Create new account" button_class="" class="" ]',
			'post_name'      => 'register',
			'post_author'    => $current_user->ID,
			'comment_status' => 'closed',
			'ping_status'    => 'closed',
		...
		$pages['page_logout'] = array(
			...
			'post_content'   => '[cjfm_logout redirect="" type="direct-logout" button_text="Logout" button_class="" class="cjfm-login-form" ]This content will be displayed to the user if type is set to message.[/cjfm_logout]',
			'post_name'      => 'logout',
			'post_author'    => $current_user->ID,
			'comment_status' => 'closed',
			'ping_status'    => 'closed',
		...
		$pages['page_reset_password'] = array(
			...
			'post_content'   => '[cjfm_form_reset_password user_login_label="Username or Email address" required_text="(required)" button_text="Continue" button_class="" class="" ]',
		...
		$pages['page_profile'] = array(
			...
			'post_content'   => '[cjfm_user_profile form_id="1" button_text="Update Profile" button_class="" class="" ]',
			'post_name'      => 'profile',
			'post_author'    => $current_user->ID,
			'comment_status' => 'closed',
			'ping_status'    => 'closed',
...
// Replace WordPress Gravatar with social profile photo
function cjfm_gravatar_social_filter($avatar, $id_or_email, $size, $default, $alt) {
	$user = false;
    if ( is_numeric( $id_or_email ) ) {
        $id = (int) $id_or_email;
        $user = get_user_by( 'id' , $id );
    ...
    if ( $user && is_object( $user ) ){
    	...
    	$social_avatar = cjfm_user_info($user->data->ID, 'cjfm_social_avatar');
    	if ($social_avatar){
    		$return = '<img src="'.$social_avatar.'" width="'.$size.'" height="'.$size.'" alt="'.$alt.'" class="avatar avatar-'.$size.' photo" />';
    	}elseif ($avatar) {
    		$return = $avatar;
    	}else{
    		$return = '<img src="'.$default.'" width="'.$size.'" height="'.$size.'" alt="'.$alt.'" class="avatar avatar-'.$size.' photo" />';
...
if(cjfm_get_option('user_avatar_type') == 'social'){
	add_filter('get_avatar', 'cjfm_gravatar_social_filter', 10, 5);
}
...
function cjfm_process_registration_form_invitations($form_id){
	...
	if(is_null($errors)){
		...
		if(!is_null($invitation_check) && $invitation_check->invited == 1){
			...
			$content_above = cjfm_get_option('outgoing_email_content_above');
			$content_below = cjfm_get_option('outgoing_email_content_below');
			cjfm_email($email_data, $content_above, $content_below);
		...
		$content_above = cjfm_get_option('outgoing_email_content_above');
		$content_below = cjfm_get_option('outgoing_email_content_below');
		cjfm_email($admin_invitation_email_data, $content_above, $content_below);
...
function cjfm_process_registration_form_normal($form_id, $redirect){
	...
	if(!is_null($errors)){
		...
		if($must_verify_email == 'enable' && cjfm_get_option('register_type') == 'normal'){
			...
			$content_above = cjfm_get_option('outgoing_email_content_above');
			$content_below = cjfm_get_option('outgoing_email_content_below');
			cjfm_email($verify_email_data, $content_above, $content_below);
		...
		if(cjfm_get_option('register_type') == 'approvals'){
			...
			$content_above = cjfm_get_option('outgoing_email_content_above');
			$content_below = cjfm_get_option('outgoing_email_content_below');
			cjfm_email($admin_email_data, $content_above, $content_below);
