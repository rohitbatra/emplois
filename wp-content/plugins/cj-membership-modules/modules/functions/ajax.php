...
function cjfm_ajax_validation_login_form() {
	...
	$results = $wpdb->get_results("SELECT * FROM wp_cjfm_temp_users WHERE user_email ='".$pdata['login_form_user_login']."'", OBJECT );
	if($user_login == '' && $user_pass == ''){
		echo cjfm_show_message('error', __('Missing required fields.', 'cjfm'));
	}elseif($results){
		echo cjfm_show_message('error', __('Your email is registered but not verified.', 'cjfm'));
	}
...
function cjfm_ajax_validation_register_form() {
	...
	// Required fields
	$required = '';
	$required_fields = $wpdb->get_results("SELECT unique_id FROM $fields_table WHERE form_id = '{$cjfm_form_id}' AND required = 'yes' AND register = 'yes' AND field_type NOT IN('heading', 'paragraph', 'custom_html', 'user_avatar') ORDER BY sort_order ASC");
	...
	// Username Checks
	$user_login = (is_null($user_login)) ? cjfm_create_unique_username($user_email) : $user_login;
	if(isset($user_login) && is_null($user_login)){
	...
	if(is_null($errors)){
		$filter_errors = apply_filters('registration_errors', $pdata);
	...
	if(is_null($errors)){
		...
		if($must_verify_email == 'enable' && cjfm_get_option('register_type') == 'normal'){
			...
			echo cjfm_show_message('success', __('We\'ve sent an email with a confirmation link to your email address.<br>Please check your email in next 5 min (also spam folder) in order to activate your account.', 'cjfm'));
		}else{
